...
import { parseCreatorManifest } from "../src/domain/creatorManifest.js";
import { normalizeCreatorResult } from "../src/domain/creatorResult.js";
...
test("normal result preserves declared facts and remains reward eligible", () => {
  const normalized = normalizeCreatorResult(manifest, {
    outcome: "success",
    score: 80.4,
    progression: { value: 3 },
    metrics: { kills: 4 },
...
test("reject range and undeclared facts reject the entire result", () => {
  assert.deepEqual(normalizeCreatorResult(manifest, { progression: { value: 99 } }).valid, false);
  assert.deepEqual(normalizeCreatorResult(manifest, { metrics: { coins: 1 } }).valid, false);
  assert.deepEqual(normalizeCreatorResult(manifest, { outcome: "win" }).valid, false);
});
