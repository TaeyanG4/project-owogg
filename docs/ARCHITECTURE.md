...
- 홈과 전체 게임 목록은 공개 catalog의 서버 업로드 시각·고유 플레이 사용자·현재 북마크 수를 이용해
  ...
  만들지 않습니다.
- 홈(`/`)은 최대 두 줄의 탐색·최근 플레이·즐겨찾기 미리보기이고, 전체 목록(`/games`)은 검색·정렬과
  장르/싱글/멀티 필터를 담당하는 별도 화면입니다. 장르 그룹은 고정 allowlist가 아니라 공개 canonical의
  `game.genre`에서 동적으로 만들며 `game.tags`는 자유 문자열 보조 분류와 검색에 사용합니다.
