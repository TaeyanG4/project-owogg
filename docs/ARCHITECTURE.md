...
- 홈과 전체 게임 목록은 공개 catalog의 서버 업로드 시각·고유 플레이 사용자·현재 북마크 수를 이용해
  ...
  만들지 않습니다.
- 홈(`/`)은 최대 두 줄의 탐색·최근 플레이·즐겨찾기 미리보기이고, 전체 목록(`/games`)은 상단 전역
  검색과 정렬·장르/싱글/멀티 필터를 담당하는 별도 화면입니다. 장르 화면은 랭킹과 같은 좌측 선택 목록과
  장르/게임 통합 검색을 제공합니다. 장르 그룹은 고정 allowlist가 아니라 공개 canonical의 `game.genre`에서
  동적으로 만들며 알려진 넓은 장르는 현재 언어로 번역합니다. 기존 official의 세부 `typing`, `reaction`,
  `brain`, `skill`은 호환상 `skill-test`로 묶고, 알 수 없는 새 장르는 원문을 유지합니다. `game.tags`는
  자유 문자열 보조 분류와 검색에 사용합니다.
- 일반 데스크톱 화면의 64px 사이드바는 pointer hover나 keyboard focus에서 본문을 밀지 않고 펼쳐집니다.
  `/games/:slug` 플레이 화면만 사용자가 접고 펼치는 상태를 로컬에 저장해 플레이 영역 너비를 명시적으로
  바꿉니다. 이 route는 좌측 플레이어·우측 광고/추천·하단 게임 정보와 플레이 액션을 배치하며 추천도
  동일한 D1/B2 공개 catalog에서 계산합니다.
