import {
  ...
  OWOGG_CREATOR_MANIFEST_VERSION,
...
export { OWOGG_CREATOR_MANIFEST_FILENAME };

export class CreatorManifestValidationError extends Error {
...
function invalid(detail: string): never {
  throw new CreatorManifestValidationError(detail);
}
...
/** Parses and semantically validates an OWOGG Creator Manifest v1 value. */
export function parseCreatorManifest(value: unknown): OwoggCreatorManifest {
  ...
  if (source.schemaVersion !== OWOGG_CREATOR_MANIFEST_VERSION) {
    invalid(`schemaVersion must be ${OWOGG_CREATOR_MANIFEST_VERSION}`);
  }
  ...
  return {
    ...
    schemaVersion: OWOGG_CREATOR_MANIFEST_VERSION,
    game,
...
export function extractCreatorManifest(
  files: readonly PreparedBundleFile[],
): OwoggCreatorManifest | null {
  const file = files.find((candidate) => candidate.path === OWOGG_CREATOR_MANIFEST_FILENAME);
  if (!file) return null;
  let parsed: unknown;
  try {
    parsed = JSON.parse(new TextDecoder().decode(file.bytes));
  } catch {
    invalid(`${OWOGG_CREATOR_MANIFEST_FILENAME} is not valid JSON`);
  }
  return parseCreatorManifest(parsed);
}
