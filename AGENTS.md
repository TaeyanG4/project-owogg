...
## 작업공간 증식 방지 및 체크포인트 통합

- 새 worktree를 만들기 전에 `git worktree list --porcelain`로 기존 작업공간과 브랜치를 확인합니다.
  같은 작업을 이어갈 수 있는 기존 worktree가 있으면 반드시 재사용합니다.
- 계획·검토·문서 조회·상태 보고 또는 중단된 작업을 이어가기 위한 목적으로 새 worktree를 만들지
  않습니다. 구현 격리가 실제로 필요한 경우에도 하나의 구현 목표에는 하나의 작업 브랜치와 하나의
  활성 worktree만 둡니다. 진행 상태를 보존하려고 `*-plan`, `*-work-progress`, `*-followup` 같은 별도
  작업공간을 추가하지 않습니다.
- 이 저장소에서 사용자의 구현·수정 요청은 범위 안의 변경을 작업 브랜치에 체크포인트 커밋하고,
  로컬 검증을 통과한 기능 단위를 기존의 깨끗한 통합 worktree에서 `staging`에 병합하는 권한을
  포함합니다. 사용자가 현재 작업에서 커밋·병합을 금지하면 그 지시를 우선합니다.
- API, DB migration, UI처럼 독립적으로 검증할 수 있는 기능 단위가 끝날 때마다 다음 작업으로 넘어가기
  전에 커밋합니다. 여러 완료 기능을 장기간 미커밋 상태로 쌓아 두거나, 미커밋 변경을 보존하기 위해
  새 worktree를 만들지 않습니다.
- 체크포인트 커밋과 병합은 현재 작업에서 만든 변경만 대상으로 합니다. 다른 작업공간이나 사용자가
  만든 변경을 임의로 stage, commit, stash, reset, clean, rebase 또는 덮어쓰지 않습니다.
- 로컬 `staging` 통합과 원격 `staging` push·배포는 구분합니다. 원격 push와 Staging 배포는 위의 필수
  작업 흐름에 따라 현재 요청에서 허용된 경우에만 실행하며, `main`과 Production 승격은 항상 별도
  명시적 승인을 받습니다.
- 통합된 작업 브랜치와 worktree는 `pnpm branches:audit`으로 정리 후보를 분류해 보고합니다. 통합
  완료만으로 삭제 권한이 생기지 않으므로, 정확한 대상에 대한 승인 전에는 worktree, branch 또는
  metadata를 삭제하지 않습니다.
...
- 미커밋 변경이 있으면 다음 중 하나가 결정될 때까지 작업을 완료로 보고하지 않습니다.
  1. 필요한 변경은 검증하고, 위 체크포인트 규칙이나 현재 요청에서 커밋 권한이 확인된 경우에만 작업
     브랜치에 커밋합니다.
